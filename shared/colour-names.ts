 ...
 *
 * Stored as one string and expanded on first use: 148 object literals is a
 * lot of punctuation to get wrong, and this way the data reads like the
 * specification table it came from.
 */
const TABLE =
...
/** name → `#rrggbb`. */
export const NAMED: Record<string, string> = Object.fromEntries(
...
/** `#rrggbb` → name, for saying "that is exactly tomato". */
export const BY_HEX: Record<string, string> = (() => {
  const out: Record<string, string> = {}
  for (const [name, hex] of Object.entries(NAMED)) if (!out[hex]) out[hex] = name
  return out
})()
