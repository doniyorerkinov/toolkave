  ...
  <div class="space-y-4">
    ...
    <template v-if="result">
      <div class="rounded-lg border border-ember-200 bg-ember-50 p-4">
        <p class="text-sm text-ember-800">{{ t('age.exact') }}</p>
        <p class="mt-1 text-2xl font-semibold text-ember-900">
      ...
      <dl class="grid gap-2 sm:grid-cols-3">
        ...
        >
          <dt class="text-xs tracking-wide text-stone-500 uppercase">{{ stat.label }}</dt>
          <dd class="mt-1 text-xl font-semibold tabular-nums text-stone-900">{{ stat.value }}</dd>
        </div>
      </dl>

      <p v-if="nextBirthday" class="text-sm text-stone-600">
        {{ t('age.nextBirthday', { days: nextBirthday.days, date: nextBirthday.date }) }}
      </p>
    </template>
    <p v-else class="text-sm text-stone-500">{{ t('age.enterDates') }}</p>
  </div>
</template>
