...
<template>
  <div class="space-y-4">
    ...
    <p v-if="sourceEncoding" class="rounded-lg border border-ember-200 bg-ember-50 px-3 py-2 text-sm text-ember-900">
      {{ t('table.detectedEncoding', { encoding: sourceEncoding }) }}
    </p>

    <div v-if="sheetNames.length > 1">
      <label for="ttp-sheet" class="mb-1 block text-sm font-medium text-stone-700">{{ t('table.sheet') }}</label>
      <select
        id="ttp-sheet"
        v-model="activeSheet"
    ...
    <div v-if="grid.length" class="flex flex-wrap items-center gap-4 rounded-lg border border-stone-200 p-3">
      ...
      <label class="flex items-center gap-2 text-sm text-stone-700">
        ...
        <input
          ...
          min="6"
          max="16"
          class="w-16 rounded-lg border border-stone-300 bg-white px-2 py-1.5 text-sm"
        />
      </label>
      <label class="flex items-center gap-2 text-sm text-stone-700">
        <input v-model="header" type="checkbox" class="rounded border-stone-300" />
        {{ t('table.repeatHeader') }}
      </label>
      <label class="flex items-center gap-2 text-sm text-stone-700">
        <input v-model="zebra" type="checkbox" class="rounded border-stone-300" />
        {{ t('table.zebra') }}
      </label>
    </div>
        ...
        <table class="w-full text-left text-xs">
          <tbody>
            <tr
              ...
              :key="rowIndex"
              :class="header && rowIndex === 0 ? 'bg-stone-100 font-semibold' : rowIndex % 2 ? 'bg-stone-50' : ''"
            >
              <td
